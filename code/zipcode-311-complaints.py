...
def get_variation_with_years(df_years):
	for year, df_year in df_years.items():
		...
		df.write.csv("/user/rachna/Desktop/task3-year-{}.csv".format(year))
		print("File saved to: /user/rachna/Desktop/task3-year-{}.csv".format(year))
...
def get_variation_with_neighborhood(df_years):
	for year, df_year in df_years.items():
		...
		df.show(5)
		df.write.csv("/user/rachna/Desktop/task3-neighborhoods-{}.csv".format(year))
		print("File saved to: /user/rachna/Desktop/task3-neighborhoods-{}.csv".format(year))
...
if __name__ == "__main__":
	...
	# import the dataset
	_311_reports_df = spark.read.csv("/user/rachna/Desktop/NYCOpenData/erm2-nwe9.tsv.gz", header='True', inferSchema='True', sep='\t')
	df = _311_reports_df 
